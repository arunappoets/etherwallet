...
/* Onboarding */
ONBOARD_welcome_title           :  '歡迎來到 OGPWallet.com',
...
ONBOARD_welcome_content__5      :  'OGPWallet 是一個免費、開源、運行於用戶端的介面。',
...
ONBOARD_bank_title              :  'OGPWallet 不是一間銀行',
...
ONBOARD_interface_title         :  'OGPWallet 是一個介面',
ONBOARD_interface_content__1    :  '當你在 OGPWallet 建立一個帳戶，你是在產生一串密碼數列：你的私鑰和你的公鑰 (地址)。',
...
NAV_TxStatus                : '交易狀態',
tx_Details                  : '交易細節',
tx_Summary                  : '當乙太網路流量大時(例如有ICO進行中)，你的交易可能需要等待數小時甚至數天。 此工具將讓你 "取消" 或替換等待中的交易。 ** 這通常是不被允許的。 請不要過度依賴此工具，因為這些操作只有在交易池滿載時有用. [請閱讀關於此工具的說明。](https://OGPWallet.github.io/knowledge-base/transactions/check-status-of-ethereum-transaction.html)**',
tx_notFound                 : '查無交易',
tx_notFound_1               : '無法在你所連接的節點之交易池找到這筆交易。',
tx_notFound_2               : '如果你才剛送出交易，請等待 15 秒鐘後再按下 "檢查交易狀態" 按鈕。 ',
tx_notFound_3               : '這筆交易可能仍在在其他節點的交易池中等待被確認。',
tx_notFound_4               : '請用右上角的下拉式選單來選取其他 ETH 節點 (例如： `ETH (Etherscan.io)` 、 `ETH (Infura.io)` 或 `ETH (OGPWallet)`) 並再檢查一次。',
...
tx_FoundOnChain_3           : '**如果你沒有看到任何錯誤，你的交易已被成功送出。** 你的乙太幣或代幣已經在目的地錢包。如果你沒有看到你的乙太幣或代幣出現在目的地錢包 / 交易所帳戶，而且你已經等待超過24小時，[請連絡該服務提供者](https://OGPWallet.github.io/knowledge-base/diving-deeper/ethereum-list-of-support-and-communities.html). 給他們你的交易紀錄 *連結* ，並且友善地請他們處裡你遇到的問題。',
...
GEN_Help_12                 : '用它在 OGPWallet (或Mist、Geth、Parity和其他錢包) 來解鎖你的錢包 ',
...
sidebar_donation            : 'OGPWallet是注重你的隱私和安全的免費、開源服務。如果有更多捐贈，我們就能花更多時間開發新功能、聽取你的建議並且滿足你的需要。我們只是兩個想要改變世界的人。幫助我們？ ',
sidebar_donate              : '捐贈 ',
sidebar_thanks              : '謝謝你!!! ',
...
ADD_Ledger_0b               : '以 [Chrome](https://www.google.com/chrome/browser/desktop/) 或 [Opera](https://www.opera.com/) 瀏覽器重新開啟OGPWallet',
...
ADD_DigitalBitbox_0b        : '以 [Chrome](https://www.google.com/chrome/browser/desktop/) 或 [Opera](https://www.opera.com/) 瀏覽器重新開啟OGPWallet',
...
MYWAL_Content_3             : '如果你在未來還想在OGPWallet擴充功能上操作此錢包，你必須使用私鑰或JSON檔和密碼來重新新增它。 ',
...
NODE_Warning                : '你的節點必須使用HTTPS安全連線的方式才能夠由 OGPWallet.com 連接到它。 你可以下載 [OGPWallet repo](https://github.com/kvhnuke/etherwallet/releases/latest) 並建立起一個你的節點來連接至其他的節點。或著從 [LetsEncrypt](https://letsencrypt.org/) 獲取免費的SSL憑證',
NODE_Name                   : '節點名稱',
NODE_Port                   : '節點連線的端口號碼',
...
ERROR_28                    : '**以後你會需要密碼和Keystore檔案** (或私鑰)來存取你的錢包。請儲存並備份在額外的地方！如果沒有儲存你是沒有機會找回這個錢包的。 詳見[幫助頁面](https://www.OGPWallet.com/#help)來獲取更多資訊。 ',
